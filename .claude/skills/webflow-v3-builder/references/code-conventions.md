...
## Rich Text PROPS are slotted light DOM (verified on published ACSI HTML, 2026-09-23)
A `props.RichText` value is NOT rendered inside the shadow root. Webflow SSRs the component's own JSX into the shadow template with a `<slot name="<propKey>">` where React placed `{content}`, and delivers the actual rich text as light DOM: `<div class="w-richtext" slot="content">…</div>`. **Consequence: shadow-CSS per-tag rules (RichText's RT_TAGS `.rt p / .rt ul` etc.) can never style prop-delivered content in production** — `::slotted()` reaches only the wrapper, not descendants. They do work in the local preview (content passes as plain React children), which is exactly how this stayed hidden. Styling for rich-text prop content must live in PAGE CSS, and the LIBRARY ships it itself via `library.globals` (the only stylesheet route): `src/styles/slotted-rich-text.css`, imported by `src/globals.ts`, carries the full per-tag rule set scoped to `.w-richtext[slot]` — only rich text inside code components (any component's rich text prop, incl. Accordion's Body), nothing else on any site. CORRECTION 2026-09-23: an earlier fix put these rules in the v1 framework repo's `richtext-extras.css` — reverted on user instruction; the v1 CDN is not the layer for v3 styles. Keep the CSS file in step with RT_TAGS (cross-referencing comments in both). Custom properties set on `.rt` (--rt-fs/--rt-lh Size override, --rt-hr-*) inherit across the slot boundary into the light DOM, so mode-aware values can be computed in the component and consumed by the page CSS; static page CSS alone cannot see the article mode (article-mode slotted content currently gets standard variables — open gap).
